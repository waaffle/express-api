...
const UserController = {
    ...
    getUserById: async (req, res) => {
        ...
        try {
            ...
            const isFollowing = await prisma.follows.findFirst({where: {
                AND: [
                    {followerId: userId},
                    {followingId: id}
                ]
        ...
        try {
            if (email) {
                ...
                if (existingUser && existingUser.id !== id) {
                    return res.status(403).json({error: "Почта уже занята"});
                }
            }
            const user = await prisma.user.update({where: {id},
                data: {
                    ...
                    name: name || undefined,
                    dateOfBirth: dataOfBirth || undefined,
                    bio: bio || undefined, 
                    location: location || undefined,
                    avatarUrl: filePath || undefined
                }});
                res.json(user);

        } catch (error) {
            console.error("Errror in updateUser", error);
            return res.status(500).json({error: "Internal server error"})  
        }
    ...
    current: async (req, res) => {
        try {
            ...
            console.error("Errror in current", error);
            return res.status(500).json({error: "Internal server error"})  
        }
